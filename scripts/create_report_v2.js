    ...
    sections: [
        ...
        // Main Content
        {
            ...
            children: [
                ...
                createParagraph("VITS（Variational Inference with adversarial learning for end-to-end Text-to-Speech）是一个基于变分推断和对抗学习的端到端语音合成模型，由 Kim 等人于 2021 年提出。该模型的核心思想是将条件变分自编码器（CVAE）与生成对抗网络（GAN）相结合，通过联合优化的方式实现高质量语音的生成。VITS 模型主要由以下几个核心组件构成："),
                createParagraph("文本编码器（Text Encoder）负责将输入的音素序列转换为连续的隐表示。该模块采用 Transformer 架构，包含多头自注意力机制和前馈神经网络层。文本编码器通过学习音素到隐空间的映射关系，捕捉文本中的语义和韵律信息。具体而言，输入的音素序列首先通过嵌入层转换为向量表示，然后经过多层 Transformer 编码器进行特征提取，最终输出文本的隐表示。"),
                ...
                new Table({
                    ...
                    rows: [
                        ...
                        new TableRow({
                            children: [
                                createCell("音频格式", { width: 3120 }),
                                createCell("WAV (16-bit PCM)", { width: 6240 })
                            ]
                        }),
                        new TableRow({
                            children: [
                                createCell("元数据格式", { width: 3120 }),
                                createCell("CSV (竖线分隔)", { width: 6240 })
                            ]
                        })
                    ]
                }),
                ...
                new Table({
                    ...
                    rows: [
                        ...
                        new TableRow({
                            children: [
                                createCell("环境管理", { width: 3120 }),
                                createCell("Conda", { width: 6240 })
                            ]
                        })
                    ]
                }),
                ...
                createParagraph("finetune.py 脚本用于在预训练模型的基础上进行微调训练："),
                ...
                createParagraph("项目的目录结构如下：configs 目录存放模型配置文件；data 目录存放数据集；models 目录存放训练好的模型；outputs 目录存放生成的音频；scripts 目录存放各种脚本文件。其中，scripts 目录包含以下主要脚本：download_data.sh 用于下载数据集，train.py 用于模型训练，finetune.py 用于模型微调，inference.py 用于推理生成，evaluate.py 用于评估计算，quick_demo.py 用于快速演示。"),
            ]
        }
    ]
});
